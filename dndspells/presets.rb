...
domains.each do |domain|
  ids = spells.select { |sp| sp['class'].include? ('Cleric') || sp['cleric_domain'].include?(domain) }.map { |sp| sp['id'] }
...
circles.each do |circle|
  ids = spells.select { |sp| sp['class'].include? ('Druid') || sp['druid_circle'].include?(circle) }.map { |sp| sp['id'] }
...
oaths.each do |oath|
  ids = spells.select { |sp| sp['class'].include? ('Paladin') || sp['paladin_oath'].include?(oath) }.map { |sp| sp['id'] }
